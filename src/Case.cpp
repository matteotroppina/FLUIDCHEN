...
Case::Case(std::string file_name, int argn, char **args) {
    ...
    if (not _grid.inflow_cells().empty()) {
        _boundaries.push_back(std::make_unique<FixedVelocity>(_grid.inflow_cells(), UIN, VIN));
    }
    if (not _grid.outflow_cells().empty()) {
        _boundaries.push_back(
            std::make_unique<ZeroGradient>(_grid.inflow_cells(), -UIN, -VIN)); // does this make sense??
    }
}
