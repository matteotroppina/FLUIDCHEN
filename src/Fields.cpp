...
Fields::Fields(double nu, double dt, double tau, int imax, int jmax, double UI, double VI, double PI) //",double alpha, double beta, double GX, double GY" in brackets, also in next line _alpha(alpha), _beta(beta), _gx(GX), _gy(GY)
...
void Fields::calculate_fluxes(Grid &grid) {
    for (int i = 1; i <= grid.size_x() - 1; i++) {
        for (int j = 1; j <= grid.size_y(); j++) {
            _F(i, j) = _U(i, j) +
                       _dt * (_nu * (Discretization::laplacian(_U, i, j)) - Discretization::convection_u(_U, _V, i, j)); // - _beta * _dt/2 * (T(i,j)+T(i+1,j)) * _gx
        }
    }
    ...
    for (int i = 1; i <= grid.size_x(); i++) {
        for (int j = 1; j <= grid.size_y() - 1; j++) {
            _G(i, j) = _V(i, j) +
                       _dt * (_nu * (Discretization::laplacian(_V, i, j)) - Discretization::convection_v(_U, _V, i, j)); // - _beta * _dt/2 * (T(i,j)+T(i,j+1)) * _gy
        }
    }
}
...
void Fields::calculate_dt(Grid &grid) {
    ...
    _dt = std::min({conv_cond, cfl_x, cfl_y}); //add new_cond in bracket
